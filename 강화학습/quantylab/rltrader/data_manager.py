...
temp = pd.read_csv('./data/csv/005930.csv', encoding='utf-8-sig')
temp.drop(columns='Unnamed: 0', inplace=True, axis=1)
COLUMNS_TRAINING_DATA_V3 = list(temp.columns)
...
def load_data_v3_v4(code, date_from, date_to, ver):
    ...
    for filename in os.listdir(os.path.join(settings.BASE_DIR, 'data', ver)):
        if filename.startswith(code):
            ...
            #     os.path.join(settings.BASE_DIR, 'data', 'csv', filename), ####### 수정 ver -> 'csv'
            #     thousands=',', header=0, converters={'date': lambda x: str(x)})
            df_stockfeatures = pd.read_csv('./data/csv/005930.csv', encoding='utf-8-sig')
            df_stockfeatures = df_stockfeatures.apply(pd.to_numeric)
            df_stockfeatures = df_stockfeatures.astype({'날짜':'str'})
            df_stockfeatures.rename(columns={'날짜' : 'date'}, inplace=True)
            df_stockfeatures.drop(columns='Unnamed: 0', inplace=True, axis=1)
            break
