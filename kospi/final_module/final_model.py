...
class DataFrameCreate(DBNetwork):
    ...
    def get_sqlalchemy_stock_investing_merge_df(self):
        ...
        for col in shift_list:
            complete_df[col] = complete_df[col].shift(381) # 하루치가 381개임
        complete_df.dropna(inplace=True)
        
        if not os.path.isdir('../../data/pickle_complete'): # 폴더가 없으면 생성해라
    ...
    def get_corr_list(self):
        ...
        check = False
        if len(pkl_list) >= 1: # 폴더 내 파일이 있는 경우
            ...
            for i in range(len(pkl_list)):
                ...
                if (stock_type == self.stock_type) and (stock_day == self.today) and (stock_period == self.period):
                    check = True
            # check 가 True 면 새로 생성하지 않고 기존 데이터 불러오기
            if check == True:
        ...
        drop_list = [] # 상관계수가 낮아 drop할 리스트 목록
        ...
        essential_list = ['날짜','시간','시가','고가', '저가', '종가', '거래량','거래대금','누적체결매도수량','누적체결매수수량','년','월','일']  # 필수로 있어야하는 리스트
        ...
        # 한번 걸러낸 리스트에서 pct_label과의 상관계수 추출
        ...
        new_corr= new_corr.drop('날짜') # 날짜은 index로 사용하므로 상관계수에서 필요없음 -> 년, 월, 일이 따로 존재함
        complete_corr = new_corr[(new_corr.values>self.corr) | (new_corr.values<-self.corr) |
                        ...
                        (new_corr.index == '누적체결매도수량') | (new_corr.index == '누적체결매수수량') | (new_corr.index == '년')| 
                        (new_corr.index == '월') | (new_corr.index == '일') ]
...
class LstmNetwork(DataFrameCreate):
    def __init__(self, *args, **kwargs):
        ...
        for md in model_list:
            ...
            if md == f'{self.stock_type}_{self.today}_{self.period}_lstm_{self.epochs}ep_{self.batch_size}bs_{self.patience}pa_{self.corr}newcor.hdf5':
                check = True
        print(check) # check = False 면 모델 학습,   check = True 면 모델, 상관계수 불러오기
        if check == False:
            self.lstm_model()
        else:
    ...
    def lstm_model(self):
        ...
        model.compile(loss='categorical_crossentropy', optimizer=optimizers.Adam(
                      learning_rate=self.learning_rate, decay=self.decay), metrics=['accuracy'])

        history=model.fit(X_train, y_train, epochs=self.epochs, batch_size=self.batch_size,verbose=1
