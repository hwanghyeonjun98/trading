...
def get_krx_stock_list(path):    
    ...
    # 파일의 크기를 비교해서 파일 크기가 더 큰 경우 kospi, 작은 경우 kosdaq
...
def update_pymysql_exist(code, type, conn):
    ...
    if (str(max_day) != today):
        ...
        while True:
            ...
            start_day = datetime.strftime(datetime.strptime(start_day, day_format) + timedelta(days=1), day_format) # 하루씩 데이터를 수집하기 위해 
            transfer_end_day = datetime.strftime(datetime.strptime(today, day_format), day_format) # 업데이트 마지막 날
            time.sleep(uniform(0.15, 0.3)) # uniform : 소수점 random
...
# 삼성전자 종목 업데이트 우선 수행 후 template 생성 -> 모든 날짜를 가지고 있는 데이터 프레임 생성을 위해
...
def update_label(concat_df):
    ...
    update_stock_info['pct_label'] = update_stock_info['pct_label'].round(1) # round : 절사
...
def db_list_update(krx):
    ...
    pct_error_list = [] # 에러가 발생한 리스트 목록 생성
    ...
    # exist_list : DB내에 존재하는 리스트, empty_list: DB 내에 존재하지 않는 신규 리스트

    template = save_samsung_template(DBConnection_stock().get_pymysql_connection())
    # template : 날짜만 있는 빈 리스트
    ...
    if len(empty_list) > 0 : # 신규 리스트가 있는 경우에만 실행
    
        error_list = [] # 에러가 발생한 리스트 목록 생성
