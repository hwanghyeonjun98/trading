from final_module.final_dbconnect import DBConnection, DBConnection_target
...
# 현재 DB 내 존재하는 테이블(종목) 추출 - 대신증권 데이터
...
# 현재 DB에 있는 investing 데이터 테이블의 가장 최신 것만 추출 -> concat
...
# 타겟 리스트 DB 저장 및 investing data 생성
def get_target(path):
    # 대상 종목 설정
    now = datetime.now()
    if (now.hour < 16):
